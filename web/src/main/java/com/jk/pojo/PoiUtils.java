...
public class PoiUtils {
	...
	/**
	 * <pre>importExcel(poi导入excel)【导入：用工具类】
	 * 创建人：wdd    
	...
	public static List importExcel(MultipartFile filename,String[] fieldarr,Class cc){
		...
		try {
			...
			//2、创建excel的工作表
			//获取上传的文件名称
			String oldname = filename.getOriginalFilename();//店铺表格.xlsx
			//判断是03还是07
			Workbook book = null;
	...
	/**
	 * <pre>exportExcel(poi导出excel)  【导出：自己写】
	 ...
	 * 				营业状态---t_eating_is
	 * 
	 * 				{"店铺id","店铺名称","营业状态"}--->{"t_eating_id","t_eating_name","t_eating_is"}
...
*/        
        ...
        for(int j = 0; j < dataAraay.size(); j++){
            ...
            // //{"id","name","other","sexname","cityname"}
            //list :{"店铺id","店铺名称","营业状态"}--->{"t_eating_id","t_eating_name","t_eating_is"}
            for(int k = 0; k < list.size(); k++){
            	String field = list.get(k);//t_eating_id、t_eating_name
            	String value = jsonObject.getString(field);//1、店铺一
